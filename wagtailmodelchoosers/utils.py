...
def get_chooser_options(chooser):
    ...
    if not chooser_options:
        raise ImproperlyConfigured(
            "Missing options definition for chooser `{}` in `MODEL_CHOOSERS_OPTIONS`".format(
                chooser
            )
        )
...
def get_response_keys_map(resource_options):
    return {
        ...
        "num_pages": resource_options.get(
            "remote_response_pages_count_key", "num_pages"
        ),
        "next": resource_options.get("remote_response_next_page_key", "next"),
        "previous": resource_options.get(
            "remote_response_previous_page_key", "previous"
        ),
